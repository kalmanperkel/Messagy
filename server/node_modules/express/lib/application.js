...
app.defaultConfiguration = function defaultConfiguration() {
  ...
  this.set('view', View);
  this.set('templates', resolve('templates'));
  this.set('jsonp callback name', 'callback');

  if (env === 'production') {
    this.enable('templates cache');
  }
...
/**
 * Render the given templates `name` name with `options`
...
app.render = function render(name, options, callback) {
  ...
  if (renderOptions.cache == null) {
    renderOptions.cache = this.enabled('templates cache');
  }
  ...
  if (!view) {
    ...
    view = new View(name, {
      defaultEngine: this.get('templates engine'),
    ...
    if (!view.path) {
      var dirs = Array.isArray(view.root) && view.root.length > 1
        ...
        : 'directory "' + view.root + '"'
      var err = new Error('Failed to lookup templates "' + name + '" in templates ' + dirs);
      err.view = view;
      return done(err);
    }
